...
public class ChessBoard {
	...
	static void findPlayers() {
		...
		for (ChessBoard board : boards) {
			if (board.started) continue;
			for (int i = 0; i < 2; i++) {
				if (board.players[i] != null) continue;
	...
	Location getPieceLocation(int x, int y) {
		return new Location(corners[0].getWorld(), 
				corners[0].getBlockX() + ((flipped ? x : y) * scale) + scale/2.0f,
				corners[0].getBlockY() - 0.3f,
				corners[0].getBlockZ() + ((flipped ? y : x) * scale) + scale/2.0f);
	}
		...
		for (int i = 0; i < scale; i++) {
			for (int j = 0; j < scale; j++) {
				locations.add(new Location(
						corners[0].getWorld(),
						corners[0].getBlockX() + scale*(flipped ? x : y) + i,
						corners[0].getBlockY(),
						corners[0].getBlockZ() + scale*(flipped ? y : x) + j));
			}
		}
		return locations;
	}
	
	//Checks if a given world location is on a given side of the board (to join or leave)
	boolean onSide(Location location, int side) {
		...
		if (flipped) //idk man good luck with this one
			return location.getBlockY() > corners[0].getBlockY() && location.getBlockY() < corners[0].getBlockY() + 4
					...
					&& ((side == 0 && location.getBlockZ() >= corners[0].getBlockZ() && location.getBlockZ() <= corners[0].getBlockZ() + half) 
					|| (side == 1 && location.getBlockZ() <= corners[1].getBlockZ() && location.getBlockZ() >= corners[1].getBlockZ() - half));
		else 
			return location.getBlockY() > corners[0].getBlockY() && location.getBlockY() < corners[0].getBlockY() + 4
				&& location.getBlockZ() >= corners[0].getBlockZ() && location.getBlockZ() <= corners[1].getBlockZ()
				&& ((side == 0 && location.getBlockX() >= corners[0].getBlockX() && location.getBlockX() <= corners[0].getBlockX() + half) 
				|| (side == 1 && location.getBlockX() <= corners[1].getBlockX() && location.getBlockX() >= corners[1].getBlockX() - half));
	} //onSide
	...
	void destroyPieces(int color) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (pieces[i][j] == null || pieces[i][j].getColor() != color) continue;
				pieces[i][j].destroy();
			}
		}
	}
	...
	void selectedBlock(UUID sender, Location blockLocation) {
		if (!isTurn(sender) || selectedPiece == null || !bothOnline()) return;
	...
	void selectedPiece(UUID sender, ChessPiece piece) {
		if (!started || !isTurn(sender) || state != 2 || !bothOnline()) return;
		if (piece.getColor() == turn) {
			for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) 
				if (pieces[i][j] != null && pieces[i][j] != piece) pieces[i][j].select(false);
			if (piece.select())
				selectedPiece = piece;
	...
	public static void inventoryInteract(Player player, int slot) {
		...
		if (board == null) return;
		for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) 
			if (board.pieces[i][j] != null) board.pieces[i][j].promote(slot);
	}
	...
	public static void inventoryClosed(Player player) {
		...
		if (board == null) return;
		for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) 
			if (board.pieces[i][j] != null) board.pieces[i][j].cancelPromote();
	}
	...
	public void movePiece(ChessPiece piece, Coord2D newLocation) {
		...
		if (pieces[newLocation.x][newLocation.y] != null) {
			...
			notation = "x";
			if (piece.getType() == 0) notation = locationFile(piece.getLocation()) + notation;
		} 
		...
		notation = castled != "" ? castled : (promoted == "" ? typeCharacter(after.getType()) : "") + notation 
				+ locationFile(newLocation) + locationRank(newLocation) + promoted;
	...
	public ChessPiece[][] getBoardCopy() {
		...
		for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) 
			if (pieces[i][j] != null) copy[i][j] = new ChessPiece(pieces[i][j]);
		return copy;
	}
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				...
				if (piece != null && piece.getColor() == color && piece.getType() == 4)
					...
					for (Coord2D v : piece.validMoves(boardToCheck, false)) opponentMoves.add(v);
			}
		}
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				...
				if (piece != null && piece.getColor() == color)
					for (Coord2D v : piece.validMoves(boardToCheck, true)) allValidMoves.add(v);
			}
		}
	...
	void createPieces(int color, boolean allPawn) {
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				if (y == color*7) {
					int pieceType = x == 0 || x == 7 ? 1 : x == 1 || x == 6 ? 3 : x == 2 
							|| x == 5 ? 2 : x == 3 ? 4 + (flipped ? 1 : 0) : 5 - (flipped ? 1 : 0);
					if (pieceType != 4 && allPawn) pieceType = 0;
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				...
				if (p == null) System.out.print(" -- ");
						...
						.replace("C", p.getColor() == 0 ? "B" : "W")
						.replace("T", p.getType() == 0 ? "P" : 
							p.getType() == 1 ? "R" : 
								p.getType() == 2 ? "B" :
									p.getType() == 3 ? "k" :
										p.getType() == 4 ? "K" : "Q"));
	...
	public static String list() {
		...
		for (ChessBoard board : boards) list += board.name + "\n";
		return list;
	}
	...
	public static void destroyAll() {
		for (ChessBoard board : boards) board.destroy();
	}
	...
	public void destroy() {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (pieces[i][j] != null) pieces[i][j].destroy();
				pieces[i][j] = null;
			}
		}
	}
