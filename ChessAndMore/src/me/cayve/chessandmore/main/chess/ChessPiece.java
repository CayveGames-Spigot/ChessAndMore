...
public class ChessPiece {
	...
	public static Inventory[] pawnPromotionInvs = { 
			...
			Bukkit.createInventory(null, 9, 
					ChatColor.WHITE + "" + ChatColor.BOLD + TextYml.getText("pawnPromotion")) };
	...
	//Static initialize function to setup the inventory, and timer functions for all pieces
					...
					for (Coord2D stand : piece.selectable) {
						for (float i = 0; i < Math.PI*2; i+= Math.PI/10/piece.board.getScale()) {
							...
							location.getWorld().spawnParticle(
									Particle.REDSTONE, location, 1, new Particle.DustOptions(Color.MAROON, 0.5f));
						}
	...
	public static int typeIndex(String type) {
		for (int i = 0; i < pieceTypes.length; i++)
			if (type.equalsIgnoreCase(pieceTypes[i])) return i;
		return -1;
	}
	...
	public static int colorIndex(String color) {
		for (int i = 0; i < pieceColors.length; i++)
			if (color.equalsIgnoreCase(pieceColors[i])) return i;
		return -1;
	}
	...
	public static ChessPiece isPiece(ArmorStand stand) {
		for (ChessPiece piece : pieces) {
			if (piece.armorStand.equals(stand)) return piece;
		}
		return null;
	}
	...
	public void selectedOtherPiece(ChessPiece piece) {
		if (selectable.contains(piece.getLocation())) selectLocation(piece.getLocation());
	}
	
	//Attempt to promote to the selected slot
	public void promote(int slot) {
		if (attemptingPromotion == null) return;
	...
	void recursiveLine(int x, int y, boolean includeCheck, ChessPiece[][] boardToCheck) {
		...
		while (true) {
			...
			if (ChessBoard.onBoard(v) && ChessBoard.getPiece(v, boardToCheck) == null) {
				if ((includeCheck && !wouldBeInCheck(v)) || !includeCheck) validMoves.add(v);
			}
	...
	public ArrayList<Coord2D> validMoves(ChessPiece[][] boardToCheck, boolean includeCheck) {
		...
		if (type == 0) { //Pawn
			...
			//Forward two
			if (ChessBoard.onBoard(f2) && boardLocation.y == (color*5)+1 && ChessBoard.getPiece(f2, boardToCheck) == null
					&& ChessBoard.getPiece(f1, boardToCheck) == null) 
				if ((includeCheck && !wouldBeInCheck(f2)) || !includeCheck) validMoves.add(f2);
			//Forward one
			if (ChessBoard.onBoard(f1) && ChessBoard.getPiece(f1, boardToCheck) == null) 
				if ((includeCheck && !wouldBeInCheck(f1)) || !includeCheck) validMoves.add(f1);
			...
			if (ChessBoard.onBoard(f1l1) && ChessBoard.getPiece(f1l1, boardToCheck) != null 
				...
				if ((includeCheck && !wouldBeInCheck(f1l1)) || !includeCheck) validMoves.add(f1l1);
			...
			if (ChessBoard.onBoard(f1r1) && ChessBoard.getPiece(f1r1, boardToCheck) != null 
				...
				if ((includeCheck && !wouldBeInCheck(f1r1)) || !includeCheck) validMoves.add(f1r1);
			...
			if (ChessBoard.onBoard(left) && ChessBoard.getPiece(left, boardToCheck) != null 
				...
				if ((includeCheck && !wouldBeInCheck(left)) || !includeCheck) validMoves.add(new Coord2D(left.x, left.y+y));
				...
				if ((includeCheck && !wouldBeInCheck(right)) || !includeCheck) validMoves.add(new Coord2D(right.x, right.y+y));
			}
			...
			for (int i = 0; i < 8; i++) {
				if (ChessBoard.onBoard(vs[i]) && (ChessBoard.getPiece(vs[i], boardToCheck) == null 
					...
					if ((includeCheck && !wouldBeInCheck(vs[i])) || !includeCheck) validMoves.add(vs[i]);
			}
		} 
			...
			if (moveCount == 0 && boardLocation.x == 3) {
				...
				if (ChessBoard.getPiece(v1, boardToCheck) == null 
						&& ChessBoard.getPiece(v2, boardToCheck) == null
				...
				{
					if ((includeCheck && !wouldBeInCheck(v2)) || !includeCheck) validMoves.add(v2);
				} 
				if (ChessBoard.getPiece(v4, boardToCheck) == null
						&& ChessBoard.getPiece(v5, boardToCheck) == null
					...
					if ((includeCheck && !wouldBeInCheck(v5)) || !includeCheck) validMoves.add(v5);
				}
			}
		}
	...
	//Check if the player would be in check if this piece was in a specific location
