...
public class SkipBoPlayer {
  ...
  public SkipBoPlayer(Location start, Location end, UUID uuid) {
    ...
    displayName = locations[0].getWorld()
        .spawn(LocationUtil.relativeLocation(locations[0], 0, 0, 0), ArmorStand.class);
    displayName.setVisible(false);
    displayName.setGravity(false);
    displayName.setCustomNameVisible(true);
    displayName.setCustomName(TextYml.getText("playerDisplayName").replace("<player>", Bukkit.getPlayer(uuid).getDisplayName()));
    cardCount = locations[0].getWorld()
        .spawn(LocationUtil.relativeLocation(locations[0], 0, -0.2f, 0), ArmorStand.class);
  ...
  public ArrayList<SkipBoCard> Clear() {
    ...
    for (SkipBoCard card : stockPile.Clear()) cards.add(card);
    for (SkipBoCard card : hand.Clear()) cards.add(card);
    for (SkipBoStack stack : discardPiles)
      for (SkipBoCard card : stack.Clear()) cards.add(card);
    return cards;
  }
  ...
  public int Count() {
    ...
    for (SkipBoStack stack : discardPiles) {
      if (stack.Size() == 1 && stack.Peek().GetNumber() == -1) count += 0;
  ...
  public SkipBoStack DiscardHasArmorStand(ArmorStand stand) {
    for (SkipBoStack stack : discardPiles)
      if (stack.HasArmorStand(stand)) return stack;
    return null;
  }
  ...
  public SkipBoCard PopSelectedCard() {
    ...
    if (selectedCard == 0) card = stockPile.Pop();
    else 
    {
  ...
  public SkipBoCardTemplate PeekSelectedCard() {
    if (selectedCard == 0) return stockPile.Peek();
    else return discardPiles[selectedCard-1].Peek();
  }
  
  public boolean IsEmpty(boolean includeDiscard) {
    if (!includeDiscard) return stockPile.Size() == 0;
    else {
      return Count() == 0;
    }
  }
  ...
  public void Destroy() {
    ...
    stockPile.Destroy();
    hand.Destroy();
    for (SkipBoStack stack : discardPiles) stack.Destroy();
  }
