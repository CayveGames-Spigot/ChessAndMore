...
public class SkipBoBoardWizard {
  ...
  public static void Left(Player sender) {
    if (activeWizards.containsKey(sender)) {
      ...
      if (wizard.currentMessage != null) ToolbarMessage.removeMessage(sender, wizard.currentMessage);
      wizard.Destroy();
  ...
  public static void SelectedBlock(Player sender, Location location) {
    ...
    if (wizard.step % 2 == 0) LeftClick(sender);
    else {
  ...
  public static boolean LeftClick(Player sender) {
    if (!activeWizards.containsKey(sender)) return false;
  ...
  void send(ToolbarMessage.Message message, boolean queue) {
    ...
    if (!queue) ToolbarMessage.send(player, currentMessage = message);
    else ToolbarMessage.sendQueue(player, currentMessage = message);
  }
  ...
  void ProgressStep() {
    step++;
    if (step < 2) {
      send(new ToolbarMessage.Message(TextYml.getText("selectBuildPile")
          .replace("<state>", step % 2 == 1 ? TextYml.getText("stateEnd") : TextYml.getText("stateStart"))).SetPermanent(true), step == 0);
    }
    else if (step < 14) {
      ...
      if (currentMessage != null) ToolbarMessage.removeMessage(player, currentMessage);
  ...
  public static void DestroyAll() {
    for (SkipBoBoardWizard wizard : activeWizards.values()) wizard.Destroy();
  }
}
