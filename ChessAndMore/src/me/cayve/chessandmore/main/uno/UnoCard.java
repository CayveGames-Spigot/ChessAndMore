...
public class UnoCard implements Comparable<UnoCard> {
  ...
  public static class UnoCardTemplate implements Comparable<UnoCard> {
    ...
    public static boolean IsAmbiguous(UnoCardTemplate card) {
      return (card.action == null && card.number == -1) || (UnoAction.valueOf(card.action) > 0
          && UnoAction.valueOf(card.action) < 4 && card.color == null);
    }
  ...
  public static String GetName(UnoAction action, UnoColor color, int number, boolean enabled) {
    String name = "";
    if (color != null)
      name += color.GetChatColor(enabled)
          + TextYml.getText("CardNames." + color.toString().toLowerCase());
    if (action != null && action != UnoAction.Normal) {
      String actionName = TextYml.getText("CardNames." + action.toString().toLowerCase());
      name +=
          " " + (enabled ? (color == null ? ChatColor.DARK_GRAY : ChatColor.WHITE) : ChatColor.GRAY)
              + "";
  ...
  public static ItemStack SetTags(ItemStack item, UnoAction action, UnoColor color, int number,
      boolean enabled) {
  ...
  public static ItemStack GetItem(UnoAction action, boolean enabled) {
    return SetTags(GetItem(GetModelData(action), GetName(action, null, -1, enabled), enabled),
        action, null, -1, enabled);
  }

  public static ItemStack GetItem(UnoColor color, boolean enabled) {
    return SetTags(GetItem(GetModelData(color), GetName(null, color, -1, enabled), enabled), null,
        color, -1, enabled);
  }

  public static ItemStack GetItem(UnoAction action, UnoColor color, int number, boolean enabled) {
    return SetTags(GetItem(GetModelData(action, color, number),
        GetName(action, color, number, enabled), enabled), action, color, number, enabled);
  }
